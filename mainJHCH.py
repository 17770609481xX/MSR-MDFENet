...
def main(args):
    ...
    # 写入Excel文件
    train_result_ones_df.to_excel('result/JHCHTrainResultFile/train_' + args.val_name + '2.xlsx', index=False)
    test_result_ones_df.to_excel('result/JHCHTrainResultFile/test_' + args.val_name + '2.xlsx', index=False)
    return result_one_last


def multi_person_run():
    name_list = ['CZY', 'FWP', 'HHJ', 'SYL', 'WZZ', 'XCB']
    # 保存结果写入excel中
    results = []
    results1 = []
    # 遍历数组，逐个测试
    for i in range(len(name_list)):
        ...
        # parser.add_argument('--data_path', type=str, default="data/JHCHDataSet.mat")# 0.5-45HZ，选择前额四导联
        parser.add_argument('--data_path', type=str, default="data/JHCHDataSetFusion.mat")# 0.5-45HZ，选择前额四导联
    ...
    # 写入Excel文件
    result_df.to_excel('result/JHCHLOSO/JHCHLOSO2.xlsx', index=False)
